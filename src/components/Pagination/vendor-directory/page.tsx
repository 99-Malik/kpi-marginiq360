...
import DataFetchingModal from '../../Modal/ManageProducts/DataFetching/DataFetchingModal';
import DataPreviewModal from '../../Modal/vendor-directory/DataPreviewModal';
...
interface PaginationProps {
    onEditVendor?: (vendor: VendorFormData) => void;
}

function Pagination({ onEditVendor }: PaginationProps) {
    ...
    const handleUploadFile = () => {
        ...
        // After 2 seconds, close data fetching modal and open data preview modal
        setTimeout(() => {
    ...
    // Handle edit vendor
    const handleEditVendor = (vendor: { vendorName: string; category?: string; paymentTerms: string; deliveryDays: string; phone: string; contactPerson: string; address: string }) => {
        if (onEditVendor) {
    ...
    // Updated data structure to match the image with vendor information
    const vendorData = [
        {
            id: '1',
            vendorName: 'Gordon Food Service',
            email: 'gordon@foodservice.com',
            contactPerson: 'Maya Thompson',
            phone: '102-242-26426',
            address: 'USA, New York Avenue 15, Central Park',
            deliveryDays: '5 Days',
            paymentTerms: 'Cash',
            category: 'Supplies'
        },
        {
            id: '2',
            vendorName: 'US Foods',
            email: 'usfoods@supplier.com',
            contactPerson: 'Liam O\'Reilly',
            phone: '102-242-26426',
            address: 'USA, New York Avenue 15, Central Park',
            deliveryDays: '5 Days',
            paymentTerms: 'COD',
        ...
        {
            id: '6',
            vendorName: 'Ben E. Keith Company',
            email: 'ben@keith.com',
        ...
        {
            id: '7',
            vendorName: 'Reinhart FoodService',
            email: 'reinhart@food.com',
    ...
    return (
        <>
            <div className="bg-white rounded-2xl border border-[#E9EAEA]">

                <div className="flex justify-between items-center p-4 border-b border-gray-200">
                    {/* Left side: Title and Subtitle */}
                    <div className="min-w-0">
                        <h1 className="text-xl lg:text-2xl font-bold text-gray-900">Vendors</h1>
                        <p className="text-gray-500 mt-2 text-sm">
                            You can manage the Ingredients items here
                        </p>
                    </div>

                    {/* Right side: Search Bar, Filters, and Show dropdown */}
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Search..."
                                className="w-64 h-9 lg:h-10 pl-10 pr-3 rounded-lg border border-[#E9EAEA] bg-white text-sm text-gray-700 outline-none focus:ring-1 focus:ring-[#E9EAEA] focus:border-[#E9EAEA]"
                            />
                        </div>

                        {/* Filters Button */}
                        <button className="flex items-center gap-2 h-9 lg:h-10 px-4 rounded-lg border border-[#E9EAEA] bg-white text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                            <svg
                                width="20"
                                height="20"
                                viewBox="0 0 20 20"
                ...
                <div className="bg-white overflow-x-auto custom-Horizontalscroll pb-8">
                    ...
                    <table className="w-full min-w-[1000px] text-sm text-left whitespace-nowrap">
                        <thead className="bg-white text-xs  text-gray-500 font-normal tracking-wider">
                            <tr onClick={handleUploadFile} className="cursor-pointer hover:bg-gray-50 transition-colors">
                                {/* Column Headers */}
                                {['Vendor Name', 'Contact Person', 'Phone', 'Address', 'Delivery Days', 'Payment Terms', 'Action'].map((header) => (
                        ...
                        <tbody>
                            {paginatedData.map((item, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-6 text-[#686F83]">{item.contactPerson}</td>
                                    <td className="px-4 py-6 text-[#686F83]">{item.phone}</td>
                                    <td className="px-4 py-6">{item.address}</td>
                                    <td className="px-4 py-6">{item.deliveryDays}</td>
                                    <td className="px-4 py-6">{item.paymentTerms}</td>
                                    <td className="px-4 py-6">
                                        <div className="flex items-center gap-2">
                                            ...
                                            >
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <g clipPath="url(#clip0_2005_14608)">
                ...
                <div className="flex justify-between items-center p-4 border-t border-gray-200 text-sm text-gray-500">
                    ...
                    <span>
                        ...
                        {Math.min(currentPage * rowsPerPage, vendorData.length)} from{" "}
                        {vendorData.length}
                    </span>
            ...
            <DataFetchingModal isOpen={isDataFetchingOpen} onClose={handleCloseDataFetchingModal} />
            <DataPreviewModal 
                isOpen={isDataPreviewOpen} 
                onClose={handleCloseDataPreviewModal} 
