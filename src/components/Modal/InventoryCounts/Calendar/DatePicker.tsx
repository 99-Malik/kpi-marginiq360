...
export function DatePicker() {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                ...
                >
                    ...
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g clipPath="url(#clip0_2935_18288)">
            ...
            <DialogContent
                className="fixed top-1/2 left-1/2 w-auto rounded-xl bg-white border border-gray-200 shadow-xl transform -translate-x-1/2 -translate-y-1/2 p-4 z-[9999]"
            >
                <DialogTitle className="sr-only">Pick Inventory Date</DialogTitle>
                <DialogDescription className="sr-only">
                    Choose a date for inventory count. Past dates are disabled.
                </DialogDescription>

                <Calendar
                    aria-label="Pick inventory count date"
                    value={date}
                    onChange={(newDate) => {
                        setDate(newDate);
                        setOpen(false);
                    ...
                    classNames={{
                        base: "text-sm",
                        cellButton: [
                            "data-[today=true]:border data-[today=true]:border-primary data-[today=true]:text-primary data-[today=true]:font-bold",
                            "data-[disabled=true]:opacity-30 data-[disabled=true]:pointer-events-none data-[disabled=true]:text-gray-400"
                        ].join(" "),
