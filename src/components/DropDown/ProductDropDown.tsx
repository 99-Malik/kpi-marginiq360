...
export default function ProductDropdown({
  ...
  defaultValue,
  onChange,
  buttonClassName = "flex items-center justify-between w-full border border-gray-200 rounded-md h-10 px-3 text-xs text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-gray-200",
  ...
  const [menuMaxHeight, setMenuMaxHeight] = useState<number>(160); // scroll cap
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const desired = 200;
      const max = Math.max(120, Math.min(desired, below > 140 ? below : above));
        ...
        return (
          <div className="relative mt-1">
            ...
            <Portal>
              <ListboxOptions
                anchor="bottom start"
                style={{ width: menuWidth ? `${menuWidth}px` : undefined, maxHeight: `${menuMaxHeight}px` }}
                  ...
                  filteredOptions.map((opt) => (
                    <ListboxOption
                      ...
                      className={({ active, selected }) =>
                        `cursor-pointer text-xs select-none px-3 py-2 ${
